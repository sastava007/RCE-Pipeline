/**
     ...
     * @param {String} path: The current working directory where the current API folder lies
...
/**
     ...
     * @description It first prepares the Docker environment and then executes the Docker sandbox
...
DockerSandbox.prototype.prepare = function(success)
{
    ...
    exec("mkdir "+ this.path+this.folder + " && cp "+this.path+"/Payload/* " + this.path+this.folder + "&& chmod 777 " + this.path+this.folder, (st) => {
            ...
            fs.writeFile(sandbox.path+sandbox.folder+"/"+sandbox.file_name, sandbox.code, (err) => {
                ...
                {
                    console.log(sandbox.langName+" file saved!");
